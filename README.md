...
- Cuenta de Google con acceso a Google Drive y Google Colab
- El archivo CSV del dataset (`delivery_promise_challenge_dataset_final.csv`) subido a Google Drive en la ruta:
  ```
  Mi unidad / Challenge Técnico Delivery Promise / delivery_promise_challenge_dataset_final.csv
  ```

### 13.2 Pasos para reproducir

1. Subir el notebook `delivery_promise_challenge_v2.ipynb` a Google Colab:
   - Ir a [colab.research.google.com](https://colab.research.google.com)
   - `Archivo → Subir notebook → seleccionar el archivo .ipynb`

2. Ejecutar todas las celdas en orden:
   - `Entorno de ejecución → Ejecutar todo` (o `Ctrl+F9`)
   - Al ejecutar la primera celda, Google solicitará autorización para montar Drive — aceptar
